...
export default class MentionEditing extends Plugin {
	...
	init() {
		...
		const feedOfMentions =  editor.config._config.mention.feeds[0].feed;
		const mentionFeed = typeof feedOfMentions === "function" ? feedOfMentions() : feedOfMentions;
		...
		editor.conversion.for( 'downcast' ).attributeToElement( {
			model: 'mention',
			view: createViewMentionElement(mentionFeed)
		} );
...
const createViewMentionElement = (feed) => ( mention, { writer } ) => {
	...
	const statusClass = isPopulated === true ? 'populated' : 'unpopulated';
